...
<template>
	<div>
		<div class="flex space-x-3 items-center cursor-pointer" @click="open">			
			<svg class="border border-black rounded-full duration-300" :class="{'rotate-90': isOpened}"
			xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="none" stroke="#9c3030" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m9 18l6-6l-6-6"/></svg>
	
			<span>編輯預計出席名單(非必填)</span>
		</div>

		<div class="absolute bottom-60 border border-black bg-slate-900 flex flex-col items-center space-y-6 rounded-xl p-2" v-if="isOpened">
			<div class="w-[240px] h-[350px] mt-3 overflow-x-hidden flex flex-col space-y-3 items-center justify-start">
				<div class="flex items-center space-x-2 mx-5" v-for="n in attenderCount" :key="n">
					<input type="text" placeholder="輸入參加者LINE名稱" v-model.lazy="attenders[n-1]">
